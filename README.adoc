...
*abstract*='NAME'::
  A Unix domain socket not being created in the file system but solely
  distinguished by its name. Similar to *path*, this also allows placeholders.
endif::[]
...
+
The placeholders supported in <<rule-socket-path,*socketPath*>> are also
supported here.
endif::[]
