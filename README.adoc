...
+
Instead of `tcp` you can also use `stream` and instead of `udp` you can use
`dgram` or `datagram`.
...
Match an existing Unix domain socket instead of an IP socket and match its path
...
*&ast;*;; Matches zero or more characters except `/`
*&ast;&ast;*;; Matches zero or more path components, eg. `a/**/z` matches `a/z`
               as well as `a/b/z`, `a/b/cde/z`, `a/b/c/c/d/z` and so on
*[&mldr;]*;; Matches a single character via a series of either verbatim
             characters or ranges, eg. `[a-cijq-s]` matches either `a`, `b`,
             `c`, `i`, `j`, `q`, `r` or `s`
*[!&mldr;]*;; Same as above, but negates the match
*\X*;; Remove the special meaning of the character 'X', eg. `\?` matches `?`

ifndef::without-abstract[]
*from-abstract*='PATTERN'::
Similar to <<rule-from-unix,*from-unix*>> but instead of matching a path, match
an abstract socket with a name matching 'PATTERN'.
endif::[]
